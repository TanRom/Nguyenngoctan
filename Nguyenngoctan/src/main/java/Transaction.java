...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = NNT_BlockChain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < NNT_BlockChain.minimumTransaction) {
            System.out.println("Đầu vào của giao dịch quá bé: " + getInputsValue());
            System.out.println("Vui lòng nhập số lượng giao dịch lớn hơn " + NNT_BlockChain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionOutput o : outputs) {
            NNT_BlockChain.UTXOs.put(o.id , o);
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            NNT_BlockChain.UTXOs.remove(i.UTXO.id);
        }
