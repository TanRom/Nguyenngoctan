...
public class NNT_BlockChain {
    public static ArrayList<VNPT_Tan> blockchain = new ArrayList<VNPT_Tan>();
    public static int difficulty = 5;
    public static float minimumTransaction = 0.1f;
    public static Store NNT_Kho; //Kho
    public static Store NNT_Cuahang; //Cửa hàng
    ...
    public static void main(String[] args) {
        ...
        genesisTransaction = new Transaction(coinbase.publicKey, NNT_Kho.publicKey, initBalanceA, null);
        ...
        VNPT_Tan genesis = new VNPT_Tan("0");
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router trong kho là : " + NNT_Kho.getBalance());
        System.out.print("Số lượng VNPT-Net Router của Cửa hàng đang còn : ");
        float initBalanceB = sc.nextFloat();
        genesisTransaction = new Transaction(coinbase.publicKey, NNT_Cuahang.publicKey, initBalanceB, null);
        genesisTransaction.generateSignature(coinbase.privateKey);
        genesisTransaction.transactionId = "0";
        genesisTransaction.outputs.add(new TransactionOutput(genesisTransaction.reciepient, genesisTransaction.value, genesisTransaction.transactionId)); //Thêm Transactions Output
        UTXOs.put(genesisTransaction.outputs.get(0).id, genesisTransaction.outputs.get(0));
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng đang còn là : " + NNT_Cuahang.getBalance());
        VNPT_Tan block1 = new VNPT_Tan(genesis.hash);
        boolean fail = true;
        while (fail){
            ...
            sendFund = NNT_Kho.sendFunds(NNT_Cuahang.publicKey, numberTransfer);
            if (sendFund==null){
                continue;
            }else{
                fail= false;
        ...
        System.out.println("Số lượng VNPT-Net Router trong kho hiện tại là : " + NNT_Kho.getBalance());
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng hiện tại là : " + NNT_Cuahang.getBalance());
    }
    public static Boolean isChainValid() {
        VNPT_Tan currentBlock;
        VNPT_Tan previousBlock;
    ...
    public static void addBlock(VNPT_Tan newBlock) {
